...
if(preg_match("/^sqlite:/", $DB_DSN))
{
	...
	if($sample_version < 2)
	{
		$dbh->exec("insert into role(role_id, role_name) values ('1', 'Пользователь')");
		$dbh->exec("update db_sample set sample_version = 2");
		$sample_version = 2;
	}
	if($sample_version < 3)
	{
		$dbh->exec("insert into role(role_id, role_name) values ('2', 'Модератор')");
		$dbh->exec("update db_sample set sample_version = 3");
		$sample_version = 3;
	}
	if($sample_version < 4)
	{
		$dbh->exec("insert into role(role_id, role_name) values ('3', 'Администратор')");
